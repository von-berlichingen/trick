...
var BASE_TYPES = ['String', 'Boolean', 'Number', 'Date', 'Text', 'JSON'];
...
AbstractClass.exists = function exists(id, callback) {
    ...
    if (id) {
        id = getInstanceId(id);
        this.schema.adapter.exists(this.modelName, id, callback);
    } else {
        callback(new Error('Model::exists requires positive id argument'));
    }
};
...
AbstractClass.find = function find(params, callback) {
    ...
    if (typeof callback === 'undefined') {
        return new Query(this.schema.models[this.modelName], 'find', params);
    } else {
        if (stillConnecting(this.schema, this, arguments)) {
            return null;
        }
...
AbstractClass.exec = AbstractClass.run = AbstractClass.all = function all(params, callback) {
    ...
    if (typeof callback === 'undefined') {
        ...
        params = buildQuery(params, this);
        var constr = this;
        constr.schema.adapter.all(this.modelName, params, function (err, data) {
            if (data && data.map) {
                data.forEach(function (d, i) {
                    var obj = new constr;
                    obj._initProperties(d, false);
...
AbstractClass.update = function update(filter, data, options, callback) {
    ...
    if (this.schema.adapter.update) {
        this.schema.adapter.update(this.modelName, filter, data, options, function (err, affected) {
            callback(err, affected);
        }.bind(this));
    } else {
        console.log('Method update undefined for this adapter');
        return;
    }
};
