...
exports.initialize = function (schema, callback) {
    ...
    if (!opts.url) {
        ...
        var proto = opts.ssl ? 'https' : 'couchbase';
        opts.url = proto + '://' + host + ':' + port;
    }
    schema.client = new couchbase.Cluster(opts.url);
    schema.client.manager().createBucket(database, {}, function(err){
        if(err) console.log('createBucket', err)
    });
    var db = schema.client.openBucket(database)
    schema.adapter = new CouchBase(schema.client, db, callback);
};
...
CouchBase.prototype.define = function (descr) {
    ...
    return self.db.manager().insertDesignDocument('caminte_' + m, design, function (err, doc) {
...
CouchBase.prototype.create = function (model, data, callback) {
    ...
    var uid = uuid.v1();
    var key = data.id || data.sid || uid;
    return this.db.upsert(key, this.forDB(model, data), function (err, doc) {
        return callback(err, key, doc.rev || null);
    });
};
...
CouchBase.prototype.save = function (model, data, callback) {
    ...
    var uid = uuid.v1();
    var key = data.id || data.sid || uid;
    return this.db.replace(key, this.forDB(model, data), function (err, doc) {
        return callback(err, key, doc.rev || null);
    });
};
...
CouchBase.prototype.all = function (model, filter, callback) {
    ...
    return _this.db.query(query, function (err, body) {
